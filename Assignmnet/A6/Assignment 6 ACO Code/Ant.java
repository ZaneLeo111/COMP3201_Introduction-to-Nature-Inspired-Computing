...
public class Ant {
     ...
     *
     * input parameter:
     * boolean array visited[], where visited[i] == true if node i has been visited
     * by this ant
     */
    ...
    public ArrayList<Integer> getEdgeLengths(int currentNode, ArrayList<Integer> notVisited) {
        ...
        // YOUR CODE GOES HERE - REPLACE THIS DUMMY CODE
        for (Integer node : notVisited) {
            int edgeLength = sim.getEdgeLength(currentNode, node);
    ...
    /*
     * (3)
     * returns the pheromone levels of the edges leading to nodes not yet visited by
     * this ant
     *
     * input parameters:
    ...
    public ArrayList<Double> getPheromoneLevels(int currentNode, ArrayList<Integer> notVisited) {
        ...
        // YOUR CODE GOES HERE - REPLACE THIS DUMMY CODE
        for (Integer node : notVisited) {
            double pheromoneLevel = sim.getPheromoneLevel(currentNode, node);
            levels.add(pheromoneLevel);
        }
        // END OF YOUR CODE
    ...
    /*
     * (4)
     * calculates and returns the probabilities of choosing each edge that leads a
     ...
     *
     * must use the standard ACO Edge Selection formula, as provided in the class
     * slides:
    ...
    public ArrayList<Double> edgeProbabilities(ArrayList<Double> levels, ArrayList<Integer> lengths) {
        ...
        double alpha = 1.0; // Set appropriate value for alpha
        double beta = 2.0; // Set appropriate value for beta
        double denominator = 0.0;

        // YOUR CODE GOES HERE - REPLACE THIS DUMMY CODE
        for (int i = 0; i < levels.size(); i++) {
            double tau = levels.get(i);
            double eta = 1.0 / lengths.get(i);
        ...
        for (int i = 0; i < levels.size(); i++) {
            ...
            double eta = 1.0 / lengths.get(i);
            double value = Math.pow(tau, alpha) * Math.pow(eta, beta);
            double probability = value / denominator;
     ...
     *
     * input parameter:
     * probabilities: the list of probabilities returned by the getProbabilities()
     * method
     */
     ...
     *
     * input parameter:
     * cumulative: the list of cumulative proabilities returned by the
     * getCumulativeProbabilities() method
     *
    ...
    public Ant(AntSim sim, int numNodes) {
        ...
        tourLength = 500; // placeholder for now
    }
    ...
    public int tour() {
        ...
        while (nodeCount < numNodes) {
            ...
            // for each of those nodes, get the lengths and pheromone levels of the edges
            // that lead to them
        ...
        // there is one final edge to visit, which takes the ant back to its starting
        // node
